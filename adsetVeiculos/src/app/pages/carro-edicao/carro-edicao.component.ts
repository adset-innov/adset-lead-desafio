...
import { CarroService, VeiculoResponse } from '../../services/carro.service';
...
export class CarroEdicaoComponent implements OnInit {
    ...
    this.carroService.getCarroPorId(this.carroId).subscribe({
      next: (carro: VeiculoResponse) => {
        this.carroForm.patchValue({
          ...
          pacoteIcarros: this.getPacoteByPortal(carro.pacotes, 1),
          pacoteWebmotors: this.getPacoteByPortal(carro.pacotes, 2),
        });
  ...
  salvar() {
    ...
    const payload = {
      ...
      pacotes: [
        { tipoPortal: 1, tipoPacote: formValue.pacoteIcarros },
        { tipoPortal: 2, tipoPacote: formValue.pacoteWebmotors },
      ],
    };
