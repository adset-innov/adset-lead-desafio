...
namespace AdSetLead.Api.Services
{
    public class FotosService : IFotosService
    {
        ...
        public List<Imagem> PhotoPathBuilder(List<Imagem> imagens)
        {
            string path = Path.Combine(HostingEnvironment.MapPath("~/Uploads"));

            imagens.ForEach(imagem =>
            {
                imagem.Url = string.Join("", path, "/", imagem.Url);
            });
